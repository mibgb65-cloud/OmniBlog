...
一个运行在 Cloudflare Workers + D1 + R2 上的轻量个人博客，包含文章搜索与分页、草稿与发布、图片媒体库、阅读点赞，以及 RSS、站点地图和基础 SEO。前端为 React 单页应用，静态资源和 API 由同一个 Worker 提供。
...
- 登录和初始化注册按来源地址限流。
- 草稿只能由作者读取，公开列表 API 只返回已发布文章的轻量摘要。
- 图片存入 R2；媒体库会标记正在被文章引用的图片并阻止误删。
- 文章正文按 Markdown 保存和展示，不执行用户输入的 HTML。
